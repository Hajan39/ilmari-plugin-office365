...
/** A fake relay, trimmed from ilmari-plugin-smtp's own test server. */
  ...
  const server = createServer((socket) => {
    ...
    socket.on("data", (chunk) => {
      for (const line of chunk.toString("utf8").split("\r\n")) {
        ...
        if (line === "") continue;
        seen.push(line);
        if (line.startsWith("EHLO")) socket.write("250-AUTH PLAIN\r\n250 SIZE 10240000\r\n");
...
test("the smtp sign-in hands the mail to a relay and never calls Graph", async () => {
  ...
  try {
    const relayCfg = {
      signIn: "smtp",
      smtpHost: "127.0.0.1",
      smtpPort: String(relay.port),
      smtpFrom: "bot@corp.com",
      refreshToken: "",
    };
    const mail = { to: "a@corp.com", subject: "done", body: "all green" };
    const out = await nodeOf("office365-email").run(mail, ctxFor({ config: relayCfg }));

    assert.deepEqual(out, { ok: true, output: "sent to a@corp.com" });
    assert.equal(calls.length, 0); // no token request, no Graph call
    assert.ok(relay.seen.includes("MAIL FROM:<bot@corp.com>"));
    assert.ok(relay.seen.includes("RCPT TO:<a@corp.com>"));
    assert.match(relay.data, /^Subject: done$/m);

    // this relay offers no STARTTLS, so a password must not leave the machine
    const withPassword = await nodeOf("office365-email").run(
      mail,
      ctxFor({ config: { ...relayCfg, smtpUser: "bot@corp.com", smtpPassword: "pw" } }),
    );
    assert.equal(withPassword.ok, false);
    assert.match(withPassword.reason, /no STARTTLS/);
  } finally {
    relay.close();
  }
});
